...
### Merge
Merging an event allows you to combine multiple events into a single event. This is most commonly used when duplicate events are submitted. Pointslocal will attempt to prevent duplication on imports but allows users to submit duplicates if they choose to ignore the "your event may already be in our system" message. 

This is a destructive operation, which means that some information will be lost in most cases. For example, if both events have a URL but the URL is not the same, the merge process will take the first event submitted's value, destroying the second (and successive) event's value.  Update: as of October, 2016, this is not a permanent operation and can be undone.

![Managing Multiple Events](img/events_merge.png)

On the Merge Event page, you'll find all of the available data from all events on the right and the final merged event's data on the left.  By clicking the data on the right (title, description, phone, etc.), it will move that respective information into the form on the left.

![Managing Multiple Events](img/events_merge_image.png)

Images can also be selected or deselected; those not selected will be removed from the database and no longer accessible to the event unless uploaded again.

The checkbox titled "Merge Date Conflicts" indicates that any recurrences of the event that occur at the same time will be removed; in other words, if Event A had a recurrence at Jan 1, 8:00 a.m. and Event B did as well, after merge there will only be a single instance at that date and time.


After submitting a merge, you'll be taken to the new event's page.
